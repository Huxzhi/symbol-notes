...
function Toggle(props: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <div class="relative shrink-0" onClick={() => props.onChange(!props.checked)}>
      <div class={`w-9 h-5 rounded-full transition-colors cursor-pointer ${props.checked ? 'bg-(--accent)' : 'bg-(--bg-active)'}`} />
      <div class={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform pointer-events-none ${props.checked ? 'translate-x-4' : ''}`} />
    </div>
  )
}
        ...
        <div class="flex flex-1 overflow-hidden min-h-0">
          ...
          <div class="w-28 shrink-0 border-r border-(--border) py-2 overflow-y-auto">
            ...
            <Show when={pluginTabs().length > 0}>
              <div class="text-[10px] t-4 px-4 pt-3 pb-1 uppercase tracking-widest">插件设置</div>
              <For each={pluginTabs()}>
                {(tab) => (
                  <button
          ...
          <div class="flex-1 overflow-y-auto p-5 min-w-0">
            <Switch>
                    ...
                    {(t) => (
                      ...
                      >
                        <div class="flex gap-1 mb-2 justify-center">
                          <For each={t.swatch}>{(c) => <div class="w-4 h-4 rounded-full border border-white/10" style={{ background: c }} />}</For>
                        </div>
                        <div class={`text-[12px] font-medium ${draftTheme() === t.id ? 'text-(--accent)' : 't-base'}`}>{t.label}</div>
              ...
              <Match when={section() === 'files'}>
                ...
                <label class="flex items-start gap-3 cursor-pointer select-none">
                  <div class="relative mt-0.5 shrink-0">
                    <input type="checkbox" class="sr-only" checked={draftAutoTs()} onChange={(e) => setDraftAutoTs(e.currentTarget.checked)} />
                    <div class={`w-9 h-5 rounded-full transition-colors ${draftAutoTs() ? 'bg-(--accent)' : 'bg-(--bg-active)'}`} />
                  ...
                  <div>
                    ...
                    <div class="text-[11px] t-3 mt-1 leading-relaxed">
                      打开文件时若缺少 <code class="bg-(--bg-hover) px-1 mx-0.5 rounded text-[10px]">created</code> 或
                      <code class="bg-(--bg-hover) px-1 mx-0.5 rounded text-[10px]">updated</code> 字段则自动写入；每次保存时更新
                      <code class="bg-(--bg-hover) px-1 mx-0.5 rounded text-[10px]">updated</code> 为当前时间。
                    </div>
                    <div class="text-[11px] t-3 mt-1">格式：<code class="bg-(--bg-hover) px-1 rounded text-[10px]">YYYY-MM-DD HH:mm</code></div>
                    <Show when={draftAutoTs()}>
                      <div class="text-[10px] t-3 mt-2 leading-relaxed border-l-2 border-(--border-2) pl-2">
                        注：浏览器 API 仅暴露文件的修改时间，<code class="text-(--text-3)">created</code> 字段将以文件的最后修改时间作为初始值。
                      </div>
                    </Show>
                  </div>
                </label>
                ...
                <label class="flex items-start gap-3 cursor-pointer select-none">
                  <div class="relative mt-0.5 shrink-0">
                    <input type="checkbox" class="sr-only" checked={draftShowOtherFiles()} onChange={(e) => setDraftShowOtherFiles(e.currentTarget.checked)} />
                    <div class={`w-9 h-5 rounded-full transition-colors ${draftShowOtherFiles() ? 'bg-(--accent)' : 'bg-(--bg-active)'}`} />
                  ...
                  <div>
                    ...
                    <div class="text-[11px] t-3 mt-1 leading-relaxed">
                      在文件树中显示图片、PDF 等非 Markdown 文件。关闭后仅显示 <code class="bg-(--bg-hover) px-1 mx-0.5 rounded text-[10px]">.md</code> 文件。
                    </div>
                  </div>
                </label>
              </Match>
                  ...
                  <For each={SHORTCUTS}>
                    {(s) => (
                      <div class="flex items-center justify-between py-2 border-b border-(--border)]">
                        ...
                        <kbd class="text-[11px] t-2 bg-(--bg-hover) border border-(--border)] px-2 py-0.5 rounded font-mono">{s.keys}</kbd>
                      </div>
                    )}
                  </For>
                </div>
              ...
              <Match when={section() === 'plugins'}>
                <div class="text-[10px] t-3 mb-3 uppercase tracking-widest">已安装插件</div>
                <div class="flex flex-col gap-1">
                  <For each={getRegisteredPlugins()}>
                    {(plugin) => (
                      <div class="flex items-center justify-between py-2.5 border-b border-(--border)]">
                        <div>
                          <div class="text-[13px] t-base font-medium">{plugin.name}</div>
                          <Show when={plugin.description}>
                            <div class="text-[11px] t-3 mt-0.5">{plugin.description}</div>
                          </Show>
                        </div>
                        <Show
                          when={!plugin.core}
                        ...
                        >
                          <Toggle
                            checked={settingsStore.pluginStates[plugin.id] ?? plugin.defaultEnabled ?? true}
                            onChange={(v) => settingsActions.setPluginState(plugin.id, v)}
                          />
                        </Show>
                      </div>
                    )}
